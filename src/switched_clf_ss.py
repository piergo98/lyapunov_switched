...
for k in range(N):
    ...
    if k > 0:
        ...
        if idx != idx_prev:
            ...
            V_prev = Xk.T @ P_lyap[idx_prev] @ Xk
            V_curr = Xk.T @ P_lyap[idx] @ Xk
            g += [ca.reshape(V_prev - V_curr, -1, 1)]
            lbg += [0.0]
            ubg += [0.0]
    
    # Enforce CLF decay constraint within this mode
    Pk = P_lyap[idx]
    Vk = Xk.T @ Pk @ Xk
...
# Extract Lyapunov matrices (remaining elements, NM matrices of size NX*NX)
P_opt = []
idx = N
for i in range(NM):
...
print(f"  - {len(P_opt)} Lyapunov matrices (one per mode)")
...
mode_idx_0 = (0 // m) % NM
V_values[0] = x0[0]**2 * P_opt[mode_idx_0][0,0] + 2*x0[0]*x0[1]*P_opt[mode_idx_0][0,1] + x0[1]**2 * P_opt[mode_idx_0][1,1]
...
for k in range(N):
    ...
    mode_idx = ((k+1) // m) % NM  # Mode index for next node
    x = X_nodes[:, k+1]
    V_values[k+1] = x[0]**2 * P_opt[mode_idx][0,0] + 2*x[0]*x[1]*P_opt[mode_idx][0,1] + x[1]**2 * P_opt[mode_idx][1,1]
...
fig.suptitle(f'Switched System with CLF Constraints (N={N}, T={T}s, α={alpha})', fontsize=14, fontweight='bold')
...
print(f"{'='*60}")
print(f"Approach: Single shooting with CLF constraints")
