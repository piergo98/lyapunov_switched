...
#switched_model = create_simple_switched_systemSLIDING()   #the sliding mode system from the original paper, with one unstable mode and one stable mode
#switched_model = create_simple_switched_system_MARGSTAB()   #system with marg stab oscillarory modes with different ellipses
#switched_model = create_simple_switched_systemSIMPLIFIED()
switched_model = create_simple_switched_systemOSCINSTAB()

# Compute Lipschitz constants for each mode using the operator norm.
# We always use sigma_max(A), regardless of the eigenvalue locations.
K_lipschitz = []
...
N = 50      # horizon length (number of phases)
T = 3.0     # total time horizon
NM = 2  # number of subsystems (modes)
M_RK4 = 5   # RK4 sub-steps per phase

x_bound = 5.0
max_x_variation = .2*M_RK4  # Maximum allowed variation of x between RK4 nodes
...
Q = ca.diag([1.0, 1.0])
# Fixed alternation of modes at every phase (equivalent to m=1)
...
for k in range(N):
    ...
    if K_k > 1e-10:
        ...
        # (max variation intended between two NLP nodes)
...
# Slack variables for CLF decay at each phase and RK4 sub-step
...
# a) At phase transitions: V_new(x_k) <= V_prev(x_k) + eps_cont
# b) Within each phase and each RK4 sub-step: V decays
...
# Add cross-mode decay constraints at each phase transition within the trajectory loop
...
for k in range(N):
    ...
    if k > 0:
        mode_prev = (k - 1) % NM
        if mode_k != mode_prev:
            # Enforce cross-mode non-increase at node x_k:
            # x_k^T P_{mode_k} x_k <= x_k^T P_{mode_prev} x_k + eps_cont
    ...
    # Enforce CLF decay constraints within phase k at every RK4 sub-step
    ...
    for j in range(M_RK4):
        ...
        s_kj = S_clf[k, j]
        # Continuous-rate CLF decay at each RK4 sub-step: V_{j+1} - exp(-alpha*dt) V_j <= s
...
# Add periodic cross-mode non-increase at final frontier (k = N)
mode_last = (N - 1) % NM
mode_first = 0
...
# Extract CLF slack variables (next N*M_RK4 elements)
idx = N
